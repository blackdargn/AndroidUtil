...
//     canvas.save();
        ...
        if (!hasFocus()) {
            ...
            if (mCircle) {
                ...
                canvas.drawCircle(mDrawRect.left + (width / 2), mDrawRect.top + (height / 2), width / 2, mFillPaint);
            } else {
                path.addRect(new RectF(mDrawRect), Path.Direction.CW);
                mOutlinePaint.setColor(0xFFFF8A00);
                canvas.drawRect(new RectF(mDrawRect), mFillPaint);
            }
            // 区域裁剪
...
//         canvas.restore();
    ...
    public void setup(Matrix m, Rect imageRect, RectF cropRect, boolean circle, boolean maintainAspectRatio) {
        ...
        mFillPaint.setARGB(50, 0x24, 0xA0, 0xFF);
