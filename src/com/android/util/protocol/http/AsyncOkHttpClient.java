...
package com.android.util.protocol.http;
...
public class AsyncOkHttpClient {
    ...
    private long cacheSize = 4*1000*1000l;
    
    private final OkApacheClient httpClient;
     ...
     */
    public AsyncOkHttpClient(int httpPort) {
     ...
     */
    public AsyncOkHttpClient(int httpPort, int httpsPort) {
     ...
     */
    public AsyncOkHttpClient(boolean fixNoHttpResponseException, int httpPort, int httpsPort) {
     ...
     */
    public AsyncOkHttpClient(SchemeRegistry schemeRegistry) {
        ...
        httpClient.getOkHttpClient().setConnectTimeout(timeout, TimeUnit.MILLISECONDS);
        httpClient.getOkHttpClient().setReadTimeout(timeout, TimeUnit.MILLISECONDS);
        List<Protocol> protoclsList = new ArrayList<Protocol>();
        protoclsList.add(Protocol.HTTP_11);
        httpClient.getOkHttpClient().setProtocols(protoclsList);
        try {
            httpClient.getOkHttpClient().setOkResponseCache(new HttpResponseCache(AppHelper.getHttpCachePath(), cacheSize));
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
    ...
    public void setEnableRedirects(final boolean enableRedirects) {
        httpClient.getOkHttpClient().setFollowProtocolRedirects(enableRedirects);
    }
     ...
     */
    public void setProxy(String hostname, int port,final String username, final String password) {
        setBasicAuth(username, password, new AuthScope(hostname, port));
    }
    ...
    public void setSSLSocketFactory(javax.net.ssl.SSLSocketFactory sslSocketFactory) {
        this.httpClient.getOkHttpClient().setSslSocketFactory(sslSocketFactory);
    }
     ...
     */
    public void setBasicAuth(final String username, final String password, AuthScope scope) {
        OkAuthenticator authenticator = new OkAuthenticator() {
            @Override
            public Credential authenticate(Proxy arg0, URL arg1, List<Challenge> arg2) throws IOException {               
                return Credential.basic(username, password);
            }

            @Override
            public Credential authenticateProxy(Proxy arg0, URL arg1, List<Challenge> arg2) throws IOException {
                return Credential.basic(username, password);
            }
        };
        this.httpClient.getOkHttpClient().setAuthenticator(authenticator );
        if(scope != AuthScope.ANY) {
            final HttpHost proxy = new HttpHost(scope.getHost(), scope.getPort());
            final HttpParams httpParams = this.httpClient.getParams();
            httpParams.setParameter(ConnRoutePNames.DEFAULT_PROXY, proxy);
        }
    }
    ...
    public void clearBasicAuth() {
        this.httpClient.getOkHttpClient().setAuthenticator(null);
        final HttpParams httpParams = this.httpClient.getParams();
